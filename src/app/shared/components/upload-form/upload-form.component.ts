...
import { EmailNotificationService, DeclarationService } from '@app/core/services';
...
export class UploadFormComponent implements OnInit {
  ...
  uploadFile() {
    ...
    this.declarationService.upload(fileUpload).subscribe(response => {
      this.isDownloading = true;
  ...
  downloadFileTemplate() {
    this.declarationService.downloadFileTemplate(this.uploadData.declarationCode).then(response => {
      ...
      const fileName =  `Thutuc${ this.uploadData.declarationCode }_FIleMau${ subfixFile }`;
