...
@Component({
  selector: 'app-return-payment-organization',
  templateUrl: './return-payment-organization.component.html',
  styleUrls: [ './return-payment-organization.component.less' ]
})
export class ReturnPaymentOrganizationComponent implements OnInit, OnDestroy {
  ...
  handleAddEmployee() {
    ...
    this.notificeEventValidData('returnPaymentOrganization');
  ...
  saveAndView(type) {
    ...
    if (this.files.length === 0) {
      this.modalService.warning({
        nzTitle: 'Chưa đính kèm file Sổ BHXH (scan) hoặc  Văn bản chứng thực hoặc bản kèm theo bản chính Giấy chứng tử đối với trường hợp chết (scan) '
      });
      return;
    }
