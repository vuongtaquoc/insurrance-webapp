...
import { TABLE_ADJUST_HEADER_COLUMNS, TABLE_ADJUST_NESTED_HEADERS } from '@app/modules/declarations/data/pending.data';
...
export class PendingComponent extends GeneralBaseComponent implements OnInit, OnChanges, OnDestroy {
  panel: any = {
  ...
  ngOnInit() {
    ...
    this.initializeTableColumns(TABLE_ADJUST_NESTED_HEADERS, TABLE_ADJUST_HEADER_COLUMNS, 'pending',currentCredentials);
    forkJoin([
      this.cityService.getCities(),
      this.nationalityService.getNationalities(),
      this.peopleService.getPeoples(),
      this.salaryAreaService.getSalaryAreas(),
      this.planService.getPlanShowCode('600c'),
    ...
    this.handlers.push(eventEmitter.on('tree-declaration:deleteUser', (data) => {
      this.handleUserDeleteTables(data.employee,'pending');
    }));
    this.handlers.push(eventEmitter.on('tree-declaration:updateUser', (data) => {
      this.handleUserUpdateTables(data.employee, 'pending');
    }));
  }
  ...
  ngOnChanges(changes) {
     if (changes.data && changes.data.currentValue && changes.data.currentValue.length) {
      ...
      this.declarations.pending.table = data;
      this.updateOrders(this.declarations.pending.table);
      this.updateOriginByTableName('pending');
    }
  }
