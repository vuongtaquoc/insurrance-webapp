...
@Component({
  selector: 'app-reissue-health-insurance-card',
  templateUrl: './reissue-health-insurance-card.component.html',
  styleUrls: [ './reissue-health-insurance-card.component.less' ]
})
export class ReissueHealthInsuranceCardComponent implements OnInit, OnDestroy {
  ...
  rollback() {
    this.router.navigate(['/declarations/reissue-health-insurance-card']);
  }
  ...
  deleteEmployeeInInfomation(declarations, declarationsDeleted, columns) {
    ...
    declarationsDeleted.forEach(d => {
        ...
        informations = informations.filter(info => {
            return info.employeeId  !== employeeInfo.employeeId;
          });
    });
...
reformatInformations() {
  ...
  private setDateToInformationList(records: any)
  {
    ...
    let informations = [];
    let dataMenual = [...this.informations];
    dataMenual = dataMenual.filter(p => p.planCode !== 'I');  
    employeesInDeclaration.forEach(emp => {
      emp.companyRelease = this.currentCredentials.companyInfo.name;

      if(emp.origin.declarationCode === 'I') {
        let item = this.informations.find(i => (i.planCode === 'I' && i.employeeId === emp.employeeId));
        if(item) {
