...
import { TABLE_ADJUST_HEADER_COLUMNS, TABLE_ADJUST_NESTED_HEADERS } from '@app/modules/declarations/data/pending-covid.data';
import { Subject, forkJoin } from 'rxjs';
import { eventEmitter } from '@app/shared/utils/event-emitter';

@Component({
  selector: 'app-pending-covid',
  templateUrl: './pending-covid.component.html',
  styleUrls: ['./pending-covid.component.less'],
  encapsulation: ViewEncapsulation.None
})
export class PendingCovidComponent extends GeneralBaseComponent implements OnInit, OnChanges, OnDestroy {
  panel: any = {
  ...
  ngOnInit() {
    ...
    this.initializeTableColumns(TABLE_ADJUST_NESTED_HEADERS, TABLE_ADJUST_HEADER_COLUMNS, 'pendingCovid',currentCredentials);
    forkJoin([
      this.cityService.getCities(),
      this.nationalityService.getNationalities(),
      this.peopleService.getPeoples(),
      this.salaryAreaService.getSalaryAreas(),
      this.planService.getPlanShowCode('600d'),
    ...
    this.handlers.push(eventEmitter.on('tree-declaration:deleteUser', (data) => {
      this.handleUserDeleteTables(data.employee,'pendingCovid');
    }));
    this.handlers.push(eventEmitter.on('tree-declaration:updateUser', (data) => {
      this.handleUserUpdateTables(data.employee, 'pendingCovid');
    }));
  }
  ...
  ngOnChanges(changes) {
     if (changes.data && changes.data.currentValue && changes.data.currentValue.length) {
      ...
      this.declarations.pendingCovid.table = data;
      this.updateOrders(this.declarations.pendingCovid.table);
      this.updateOriginByTableName('pendingCovid');
    }
  }
